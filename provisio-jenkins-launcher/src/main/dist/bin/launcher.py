...
def build_java_execution(options, daemon):
    ...
    classpath += ":" + pathjoin(options.install_path, 'etc')

    command = ['java', '-cp', classpath]
    command += jvm_properties + system_properties
    command += [main_class]
    if exists(options.app_params):
        command += load_lines(options.app_params)
...
def create_parser():
    ...
    parser.add_option('--app-params', metavar='FILE', help='Defaults to INSTALL_PATH/etc/app-params.config')
...
def main():
    ...
    o.app_params = realpath(options.jvm_config or pathjoin(o.install_path, 'etc/app.config'))
