...
Provisio was originally created for the [Presto](https://prestosql.io/) project to provide a common way to build Presto plugins and the Presto Server. Presto has since evolved into [Trino](https://trino.io/), and Provisio continues to be the packaging foundation for the [Trino Server](https://github.com/trinodb/trino/tree/master/core/trino-server) and [Trino connector plugins](https://github.com/trinodb/trino-maven-plugin). You'll notice how small the Trino Server [`pom.xml`](https://github.com/trinodb/trino/blob/master/core/trino-server/pom.xml) is even though, at the time of this writing, there are 40+ connectors and plugins packaged in the Trino Server build as per the [Provisio descriptor](https://github.com/trinodb/trino/blob/master/core/trino-server/src/main/provisio/trino.xml). As you'll read below, you only need to specify what you need in the Provisio descriptor and Maven will figure out the rest, correctly, without having to pollute your `pom.xml` with duplicated dependency declarations.
