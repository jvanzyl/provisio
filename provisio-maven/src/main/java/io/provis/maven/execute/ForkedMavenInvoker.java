...
public class ForkedMavenInvoker implements MavenInvoker {
  ...
  public MavenResult invoke(MavenRequest request) throws ArtifactResolutionException {
    ...
    if (request.getGlobalSettings() != null) {
      if (request.getMavenVersion().startsWith("2.")) {
        cli.createArg().setValue("-D");
        cli.createArg().setValue("org.apache.maven.global-settings=" + request.getGlobalSettings().getAbsolutePath());
      } else {
