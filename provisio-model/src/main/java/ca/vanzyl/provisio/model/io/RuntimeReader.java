...
public class RuntimeReader {
    ...
    public RuntimeReader(List<ActionDescriptor> actions, Map<String, String> versionMap) {
        ...
        xstream.allowTypesByWildcard(new String[] {
            "ca.vanzyl.provisio.model.**", "ca.vanzyl.provisio.action.**",
        });
            ...
            while (reader.hasMoreChildren()) {
                ...
                if (reader.getNodeName().equals("artifactSet")) {
                    ...
                    if (actionDescriptor != null) {
                        runtime.addAction((ProvisioningAction)
                                context.convertAnother(runtime, actionDescriptor.getImplementation()));
                    } else {
                        // We can't look up a valid action so we have an invalid element
                        throw new ConversionException(
                                "The element '" + actionName + "' is invalid inside the <runtime/> context.");
                    }
                }
                reader.moveUp();
            }
    ...
    public class ArtifactConverter implements Converter {
        ...
        public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
            ...
            if (reference != null) {
                ...
                //
                // If the user is specifying versionless coordinates they are expecting to glean the version from their
                // dependency
                // management system, like in Maven
                //
                String coordinate = reader.getAttribute("id");
                int coordinateSegments =
                        coordinate.length() - coordinate.replace(":", "").length() + 1;
                if (coordinateSegments == 2) {
                    //
                    // We only have groupId:artifactId where the extension defaults to "jar" which we need to add
                    // because the
                ...
                //
                // Look at the last element of coordinate and determine if it's a version. If it's not then we need to
                // consult
                ...
                if (!Character.isDigit(lastElement.charAt(0))) {
                    ...
                    if (version != null) {
                        ...
                        throw new RuntimeException(String.format(
                                "A version for %s cannot be found. You either need to specify one in your dependencyManagement section, or explicity set one in your assembly descriptor.",
                                coordinate));
                    }
                }
            ...
            while (reader.hasMoreChildren()) {
                ...
                if (actionDescriptor != null) {
                    artifact.addAction((ProvisioningAction)
                            context.convertAnother(artifact, actionDescriptor.getImplementation()));
                    ...
                    // We can't look up a valid action so we have an invalid element
                    throw new ConversionException(
                            "The element '" + nodeName + "' is invalid inside the <artifact/> context.");
                }
                reader.moveUp();
            }
            return artifact;
